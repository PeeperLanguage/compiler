...
func TestValidateReportsDefects(t *testing.T) {
	tests := []struct {
		name   string
		damage func(effect.Result, moduleid.FunctionID, cfg.SiteID)
		want   string
	}{
		{
		...
		},
		{
			...
			damage: func(result effect.Result, fn moduleid.FunctionID, site cfg.SiteID) {
				result[moduleid.FunctionID("missing-function")] = effect.SiteOps{}
			},
...
func anySite(t *testing.T, result effect.Result) (moduleid.FunctionID, cfg.SiteID) {
	t.Helper()
